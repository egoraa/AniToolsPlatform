...
- Публичные сущности (типы, функции, поля публичных структур) документируются
  Doxygen-комментариями `///` у объявления: первая фраза — краткое описание,
  далее по необходимости `@param`, `@return`, `@throws`.
- Внутри тел функций и у приватных членов — только короткие пояснения в
  сложных или неочевидных местах.
