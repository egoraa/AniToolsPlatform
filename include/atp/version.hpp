...
namespace atp {
    ...
    struct version {
        ...
        constexpr version(TParts... p)
            : parts{static_cast<unsigned>(p)...}, count{sizeof...(TParts)} {}

        // Лексикографически по дополненному массиву; count не участвует.
        friend constexpr std::strong_ordering operator<=>(const version& left,
                                                          const version& right) {
    ...
    namespace detail {
        ...
        struct fixed_string {
            ...
            constexpr fixed_string(const char (&text)[N]) { std::ranges::copy(text, chars.begin()); }
