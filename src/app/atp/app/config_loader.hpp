...
// Потолок вложенности — защита от патологий; осмысленные конфиги мельче.
...
inline nlohmann::json read_json(const std::filesystem::path& path) {
    ...
    try {
        ...
        // e.what() содержит позицию ошибки — путь добавляем сами
...
inline void expand_includes(nlohmann::json& node, const std::filesystem::path& dir,
    ...
    if (node.is_object()) {
        if (node.contains("$include")) {
            // Узел-инклуд замещается целиком: соседние ключи потерялись бы
            // молча — поэтому запрещены.
            if (node.size() != 1) {
            ...
            // Версия схемы объявляется один раз — корневым документом.
...
// Чтение конфига с разворачиванием {"$include": "path.json"} в любом месте
// дерева: пути — относительно включающего файла, циклы и глубина под
// контролем. Возвращается обычный JSON-документ — валидатор и decode
// работают уже без знания об инклудах.
