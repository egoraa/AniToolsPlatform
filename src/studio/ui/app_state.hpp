...
struct app_state {
    ...
    std::optional<std::filesystem::path> doc_path;  // nullopt for a new unnamed document
    session run{manager.registry()};

    std::string current_group;   // group shown by the canvas ("" is the root)
    std::string selected_child;  // selected child of that group; empty means none

    // Cache of module descriptions, so the factory is not probed on every rebuild of the scene.
    // The key is "factory@version".
    std::unordered_map<std::string, module_info> describe_cache;

    // Generation of describe_cache: it takes part in the inspector's form identity, so a reloaded
    // plugin rebuilds the property rows instead of leaving the previous description on screen.
    ...
    /// Directory the document lives in, or the current directory if it has not been saved.
    ...
    [[nodiscard]] const module_info* describe_cached(const std::string& factory, const std::optional<version>& ver) {
        ...
        if (f == nullptr) {
            return nullptr;  // no factory (the plugin is not loaded) — the node shows no ports
        }
...
/// document_changed rebuilds the dependent widgets, error writes into the log, and
