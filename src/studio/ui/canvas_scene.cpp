...
canvas_scene::canvas_scene(app_state& state, ui_callbacks& callbacks, QObject* parent)
    ...
    QObject::connect(this, &QGraphicsScene::selectionChanged, this, [this] {
        if (rebuilding_) {
            return;  // a rebuild churns the selection; that is not a user action
        }
        std::string selected;
...
void canvas_scene::rebuild() {
    ...
    if (g == nullptr) {
        // The group may have been removed or renamed — fall back to the root.
...
void canvas_scene::mousePressEvent(QGraphicsSceneMouseEvent* event) {
    // Only the left button arms a drag; the right one goes to contextMenuEvent without flashing a
    // line.
...
void canvas_scene::mouseReleaseEvent(QGraphicsSceneMouseEvent* event) {
    if (temp_link_ != nullptr) {
        ...
        if (target != nullptr && target != drag_from_ && target->is_output() != drag_from_->is_output()) {
            ...
            // Released over empty space — export the child port out of the group.
            try {
...
void canvas_scene::contextMenuEvent(QGraphicsSceneContextMenuEvent* event) {
    ...
    if (!items(event->scenePos()).isEmpty()) {
        // Over a node or a link: "new group here" would drop it on top of what is already there.
        QGraphicsScene::contextMenuEvent(event);
        return;
    }
    QMenu menu;
    QAction* group = menu.addAction(QStringLiteral("New group here"));
    if (menu.exec(event->screenPos()) == group) {
        create_group(
            state_, callbacks_,
...
void canvas_scene::keyPressEvent(QKeyEvent* event) {
    if (event->key() == Qt::Key_Delete && !state_.run.running()) {
        // Links go in descending index order: removing one shifts the rest.
        std::vector<std::size_t> link_indices;
        std::vector<std::pair<bool, std::string>> stubs;  // (is_output, alias) exports to withdraw
...
void canvas_scene::build_node(const runtime::child_node& c,
    ...
    // (port, is_output, type) — the type colours the pin, one colour per type.
