...
// The alias an item was rebuilt with: the edited text is in the item, the previous name has to come
// from somewhere for the remove half of a rename.
...
void expose_editor::sync() {
    // The lambda is dropped if the editor is gone by the time the event loop gets to it.
...
void expose_editor::rebuild(const std::string& group_path) {
    ...
    for (const auto& [alias, path] : inputs_ ? g->expose_inputs : g->expose_outputs) {
        ...
        // The exported path may be missing from the candidates when the child's plugin is not
        // loaded: keep it visible rather than silently rewriting the document.
...
void expose_editor::commit_alias(QTreeWidgetItem* item) {
    ...
    guard("expose", [&] {
        if (inputs_) {
            state_.doc.remove_expose_input(group_path_, old_alias);
            state_.doc.set_expose_input(group_path_, next, port);
        } else {
            state_.doc.remove_expose_output(group_path_, old_alias);
            state_.doc.set_expose_output(group_path_, next, port);
        }
    });
}
...
void expose_editor::guard(const char* context, const std::function<void()>& operation) {
    try {
        ...
        sync();  // the table goes back to what the document actually holds
    }
}
