...
class expose_editor final : public QWidget {
   public:
    ...
    /// Re-reads the table when the document changed under it. The rebuild is queued rather than
    /// done on the spot: sync() is reached from an edit in this very table (a cell editor's signal
    /// → document_changed → the window's refresh), and rebuilding destroys that editor — deleting
   ...
   private:
    ...
    // Applies an edited alias: the document has no rename, so the old alias is dropped and the new
    // one is put on the same port.
    ...
    // Guards the itemChanged handler while rebuild fills the table: filling emits the same signal
    // an edit does.
    bool filling_ = false;
};
...
#endif  // ATP_STUDIO_UI_EXPOSE_EDITOR_HPP
