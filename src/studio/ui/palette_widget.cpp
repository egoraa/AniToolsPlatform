...
palette_widget::palette_widget(app_state& state, ui_callbacks& callbacks, QWidget* parent)
    ...
    // The hint lives in a tooltip rather than in the column header: a header is a column name, and
    // an instruction sitting there is what made this panel look unlike the others.
...
void palette_widget::refresh() {
    clear();
    // The group is not a module and comes from no plugin, so it stands above the plugin headers as
    ...
    for (const plugin_info& p : state_.manager.plugins()) {
        if (!p.loaded) {
            continue;  // failures are shown in the Modules panel
        }
    ...
    expandAll();
    setEnabled(!state_.run.running());  // the document is read-only while running
}
...
void palette_widget::add_from_item(QTreeWidgetItem* item) {
    ...
    if (item->parent() == nullptr || item->isDisabled()) {
        return;  // a double click on a plugin header adds nothing
    }
