...
runtime_widget::runtime_widget(app_state& state, ui_callbacks& callbacks, QWidget* parent)
    ...
    // The same header the inspector's sections carry, so the two docks read as one panel language.
    ...
    QObject::connect(stop_, &QPushButton::clicked, this, [this] {
        state_.run.stop();
        callbacks_.document_changed();  // the whole window leaves read-only mode
    });
}
...
void runtime_widget::start() {
    try {
        // Config plugins resolve exactly as they do for atp_app: relative to the document's
        // directory.
        ...
        callbacks_.document_changed();  // the window goes read-only
