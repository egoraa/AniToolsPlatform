...
void thread_table::refresh() {
    ...
    if (key != signature_) {
        signature_ = key;
        // Queued rather than immediate: an edit in a cell reaches refresh() through its own signal
        // (commit → document_changed → the window's refresh), and rebuild() destroys that very
        // cell editor — deleting the sender of the signal being emitted is unsafe in Qt. The rest
        // of the pass is left to the next poll, a tenth of a second away.
    ...
    for (int row = 0; row < table_->rowCount(); ++row) {
        ...
        if (mode == nullptr || period == nullptr) {
            continue;  // between a queued rebuild and its turn in the event loop
        }
        mode->setEnabled(!running);
        // The period is only meaningful for a throttled thread, so a running pipeline is not the
        // only reason for it to be disabled.
...
void thread_table::rebuild() {
    ...
    for (std::size_t i = 0; i < threads.size(); ++i) {
        ...
        // Renaming a thread would mean dropping and re-adding it, which loses the assignments — the
        // name is fixed at declaration.
...
void thread_table::update_stats() {
    ...
    for (std::size_t i = 0; i < stats.size(); ++i) {
        // The runner reports threads in its own order; the row is found by name, since the document
        // and the run may differ (an empty thread is never created).
        for (int row = 0; row < table_->rowCount(); ++row) {
            ...
            // Load over the window between polls: deltas are more honest than the running totals.
            QString busy = "-";
