...
/// Runner threads of the document together with their live statistics. Declaring a thread and
/// watching what it does are the same question, so they share one table: the period is edited right
/// next to the load it produces. Editing is document editing, hence read-only while the pipeline
...
class thread_table final : public QWidget {
   ...
   private:
    ...
    // Everything the rows are built from, in one string: rebuilding on every poll would fight the
    // cell editors, so it happens only when this changes.
...
#endif  // ATP_STUDIO_UI_THREAD_TABLE_HPP
