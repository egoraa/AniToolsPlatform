...
class McpCatalogTools : public ::testing::Test {
   protected:
    void SetUp() override {
        ...
        // The build tree lives outside the workspace root, so the directory holding the test plugin
        // is passed as an explicit --plugin-dir — which is exactly the policy being exercised.
