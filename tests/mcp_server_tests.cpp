...
// A fixture wiring one echoing tool, one throwing tool and one resource — enough to exercise every
// branch of the dispatcher without dragging the studio in.
...
TEST_F(McpServer, ReportsAToolFailureAsIsErrorRatherThanAProtocolError) {
    ...
    EXPECT_FALSE(reply.contains("error"));  // a failing tool is still a successful response
