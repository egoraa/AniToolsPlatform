...
TEST(McpWorkspace, StartsWithAnEmptyDocumentAtTheCurrentSchema) {
    ...
    EXPECT_EQ(ws.doc().config().schema, atp::runtime::config_schema_version);
    EXPECT_FALSE(ws.document_path().has_value());
...
TEST(McpWorkspace, AllowsPluginsFromTheExplicitlyListedDirectoriesOnly) {
    ...
    // A plain config path stays confined to the root even when plugin directories are configured.
...
TEST(McpWorkspace, SavesOpensAndRemembersTheDocumentPath) {
    ...
    atp::mcp::workspace ws(root);
    ws.doc().add_group("", "stage");

    const std::filesystem::path file = ws.resolve("pipeline.json");
    ws.save_document(file);
    ASSERT_TRUE(std::filesystem::exists(file));
    ASSERT_TRUE(ws.document_path().has_value());
    EXPECT_EQ(*ws.document_path(), file);
    EXPECT_EQ(ws.document_dir(), root);

    atp::mcp::workspace reopened(root);
    reopened.open_document(file);
    ASSERT_NE(reopened.doc().group_at("stage"), nullptr);

    reopened.reset_document();
    EXPECT_EQ(reopened.doc().group_at("stage"), nullptr);
    EXPECT_FALSE(reopened.document_path().has_value());
    std::filesystem::remove_all(root);
}
