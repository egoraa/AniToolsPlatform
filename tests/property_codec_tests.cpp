...
// A user type with a codec specialisation of its own.
struct percent {
    int value = 0;
};
...
// Specialised inside atp::io, exactly as the built-in types are.
template <>
...
TEST(PropertyCodec, IntRejectsGarbage) {
    ...
    EXPECT_EQ(atp::io::property_codec<int>::from_string("12x"), std::nullopt);  // a trailing tail is a refusal too
...
TEST(PropertyCodec, BoolIsWordsTrueFalse) {
    ...
    EXPECT_EQ(atp::io::property_codec<bool>::from_string("TRUE"), std::nullopt);  // no liberties with the case
...
TEST(PropertyCodec, UserSpecializationSatisfiesConcept) {
    ...
    static_assert(!atp::io::property_value<void*>);  // without a specialisation the type is rejected
