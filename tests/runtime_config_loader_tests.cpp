...
// Every test writes into its own subdirectory of temp_directory_path, so the files never collide.
...
TEST_F(LoaderFiles, ExpandsNestedIncludesRelativeToIncludingFile) {
    ...
    write("sub/mid.json", R"({"mid": {"$include": "leaf.json"}})");  // the path is relative to sub/
...
TEST_F(LoaderFiles, RejectsVersionInsideFragment) {
    write("frag.json", R"({"version": "2.0"})");  // only the root document carries a version
