...
TEST(ConfigDecode, BuildsTypedModelPreservingOrder) {
    ...
    ASSERT_TRUE(cfg.pipeline.modules[0].module.has_value());  // the array order is preserved
    ...
    // the pair order is the object's items() order, which nlohmann keeps alphabetical
...
TEST(ConfigDecode, DefaultsNameToFactoryAndOmittedFields) {
    ...
    EXPECT_EQ(cfg.pipeline.modules[0].module->name, "counter");  // the default is the factory name
...
TEST(ConfigEncode, RoundTripsCanonicalDocument) {
    // Canonical form: defaults omitted, the mode spelled out, expose aliases in alphabetical order
    // (a JSON object sorts them) — encode produces exactly this document. Property scalars have to
    // keep their type: 10, not "10".
    const nlohmann::json doc = nlohmann::json::parse(R"({
        "version": "2.0",
        "plugins": ["p.dll"],
        "pipeline": {
...
TEST(ConfigEncode, OmitsDefaultsAndStaysValid) {
    ...
    // canonicalisation: a name equal to the factory is omitted, empty sections vanish, mode is explicit
    ...
    EXPECT_TRUE(atp::runtime::validate(out).empty());  // encode produces a valid config
}
