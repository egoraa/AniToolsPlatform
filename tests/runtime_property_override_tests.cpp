...
TEST(PropertyOverride, ValueMayContainEqualsAndDots) {
    ...
    EXPECT_EQ(o.value, "C:\\a=b\\x.txt");  // split on the FIRST '='; dots to the right belong to the value
}
...
// An enum needs no special code — the name arrives as a string and the property parses it; this
// test pins down that the hint from the message reaches -p as well.
