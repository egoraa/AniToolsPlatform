...
TEST(StudioLayout, ChainsFormColumnsByConnectionDirection) {
    ...
    ASSERT_EQ(p.size(), 3u);
    EXPECT_LT(p.at("a").x, p.at("b").x);  // the chain grows to the right
...
TEST(StudioLayout, IndependentNodesStackInFirstColumn) {
    ...
    const auto p = atp::studio::auto_layout(g);
    EXPECT_EQ(p.at("a").x, p.at("b").x);  // one layer
    EXPECT_NE(p.at("a").y, p.at("b").y);  // different rows — no overlap
}
...
TEST(StudioLayout, ConnectionCycleDoesNotHang) {
    ...
    const auto p = atp::studio::auto_layout(g);  // terminating at all is the point
    EXPECT_EQ(p.size(), 2u);
}
