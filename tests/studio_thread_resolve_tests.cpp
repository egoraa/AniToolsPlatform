...
// A config carries only what resolution looks at: the thread list and the assignments. The group
// tree is irrelevant here — resolve_thread walks paths, not nodes.
...
// The root cannot be assigned (document::set_assignment rejects an empty path), so it always falls
// back to the first declared thread — the same rule pipeline_runner applies.
...
// A prefix match must land on a path separator: "outer2" is not a child of "outer".
TEST(ThreadResolve, PrefixMatchRespectsPathSeparator) {
    ...
    EXPECT_EQ(r.name, "producing");  // the fallback, not "consuming"
    EXPECT_TRUE(r.inherited);
}
